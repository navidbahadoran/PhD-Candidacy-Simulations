...
def run(p=1000, n=500, reps=30, lambdas=(1.2, 1.6, 2.0, 2.6, 3.5, 5.0), seed=777, prefix="figures/sim0_bbp_transition"):
    ...
    gamma = p / n
    thr = 1.0 + np.sqrt(gamma) # population threshold
    b = (1.0 + np.sqrt(gamma))**2 # sample bulk edge (still useful to report)
    ...
    plt.axvline(thr, ls="--", lw=1, label=r"threshold $1+\sqrt{\gamma}$")
    plt.xlabel("population spike $\\lambda$")
    plt.ylabel("top sample eigenvalue")
    plt.title(f"BBP eigenvalue (γ={gamma:.2f}, MP edge b={(1+np.sqrt(gamma))**2:.2f})")
    ...
    plt.axvline(thr, ls="--", lw=1, label=r"threshold $1+\sqrt{\gamma}$")
    plt.xlabel("population spike $\\lambda$")
    plt.ylabel(r"alignment $|\langle \hat v, v\rangle|^2$")
    plt.title(f"BBP alignment (γ={gamma:.2f})")
    plt.legend(frameon=False)
